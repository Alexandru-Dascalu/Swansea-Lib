...
public class EventNotification extends Notification {
	 ...
	 * given event has just been added.
	...
	/**
	 * Makes the message that should be displayed for a notification when the 
	 * given event will happen in the near future.
	...
	public static String getNearingEventMsg(Event event) {
		...
		if(days > 0) {
			...
			return "You have one event that will be in less than a day: " 
					+ event.getTitle() + ".";
		}
	}
	 ...
	 * associates in the database the newly made notification with users with
	 * the new event notification setting turned on.
	...
	/**
     * Makes a new notification in the database for an event a user is attending
     * and that is close to the current date. It also associates in the database
     * the newly made notification with users with the nearing event 
	...
	/**
	 * Gets a list of users names from the database of users with
     * the new event notification setting turned on.
	 * @return list of users names of users with
     * the new event notification setting turned on.
	 */
	...
	/**
     * Gets a list of users names from the database of users with
     * the nearing event notification setting turned on.
     * @return list of users names of users with
     * the neaering event notification setting turned on.
     */
	 ...
	 * @return a string representing the date of the event associated with
	 * this notification.
	 */
     ...
     * spacing and growth policy of the new HBox. It adds the message of the 
     * notification and the event date to the HBox.
